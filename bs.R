    ...
    for (drug in drugs) {
      ...
      tryCatch({command <- paste("./gdpathways.sh ", uniProtID.drug, " > drugProteinPathways.txt ", sep="")
                ...
                if (file.exists("drugProteinPathways.txt") && file.info("drugProteinPathways.txt")$size > 0) {
                  ...
                  system(paste("cp ", "drugProteinPathways.txt ", adr, "/", ci, "/", drug, "/allDrugProteinPathways.txt", sep=""))
                  system(paste("cp ", "proteinPathways.txt ", adr, "/", ci, "/", drug, "/ixnProteinPathways.txt", sep=""))
                  system(paste("cp ", "proteinPathways.txt ", adr, "/", ci, "/", drug, "/ixnProteinPathways.txt", sep=""))
      ...
      tryCatch({print("#######################################################")
                ...
                if (file.exists("drugTargets.txt") && file.info("drugTargets.txt")$size > 2) {
                  ...
                  system(paste("cp drugTargets.txt ", adr, "/", ci, "/", drug, "/allTargetGenes.txt", sep=""))
